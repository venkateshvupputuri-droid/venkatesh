...
async function loadAssemblies() { const rows = await api(`/plans/${activePlan.PlanId}/assemblies`, {}, true); displayedAssemblies = rows; const body = el("assemblyTableBody"); let total = 0; body.replaceChildren(...rows.map((row, index) => { total += Number(row.Weight) || 0; const tr = document.createElement("tr"); tr.innerHTML = `<td>${index + 1}</td><td>${row.SequenceCode}</td><td>${escapeHtml(row.AssemblyName || row.AssemblyGuid)}</td><td>${escapeHtml(row.AssemblyMark || "—")}</td><td>${Number(row.Weight || 0).toFixed(3)}</td><td>${escapeHtml(row.Grid || "—")}</td><td>${escapeHtml(row.Tos || "—")}</td>`; return tr; })); el("assemblyEmpty").hidden = rows.length > 0; el("totalWeight").textContent = `Total weight: ${total.toFixed(3)} t`; }
