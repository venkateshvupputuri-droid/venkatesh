...
async function colourizePlan() { if (!selectedPlans.length) throw new Error("Select at least one plan first."); const rowsByPlan = await Promise.all(selectedPlans.map(plan => api(`/plans/${plan.PlanId}/assemblies`, {}, true))); await workspace.viewer.setObjectState(undefined, { color: "#8b9299", opacity: 1 }); const palette = ["#f6a623", "#4da3ff", "#d86bff", "#55c878", "#ff6f61", "#35c2c9", "#f2d04f", "#ef8f45"]; const coloured = new Set(); for (const [index, rows] of rowsByPlan.entries()) { const ids = rows.filter(row => row.ModelId && row.ObjectRuntimeId && !coloured.has(`${row.ModelId}:${row.ObjectRuntimeId}`)).reduce((groups, row) => { coloured.add(`${row.ModelId}:${row.ObjectRuntimeId}`); const group = groups.find(item => item.modelId === row.ModelId) || (groups.push({ modelId: row.ModelId, objectRuntimeIds: [] }), groups.at(-1)); group.objectRuntimeIds.push(row.ObjectRuntimeId); return groups; }, []); if (ids.length) await workspace.viewer.setObjectState(ids, { color: palette[index % palette.length], opacity: 1 }); } status(`${selectedPlans.length} plan(s) coloured; other IFC assemblies are grey.`, "success"); }
