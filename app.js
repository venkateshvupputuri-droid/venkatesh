...
function colourIds(rows) { return rows.filter(row => String(row.ModelId || currentModel.id) === String(currentModel.id) && Number.isInteger(Number(row.ObjectRuntimeId)) && Number(row.ObjectRuntimeId) > 0).reduce((groups, row) => { const group = groups[0] || (groups.push({ modelId: currentModel.id, objectRuntimeIds: [] }), groups[0]); group.objectRuntimeIds.push(Number(row.ObjectRuntimeId)); return groups; }, []); }
async function colourizePlan() { if (!selectedPlans.length) throw new Error("Select at least one plan first."); const rowsByPlan = await Promise.all(selectedPlans.map(plan => api(`/plans/${plan.PlanId}/assemblies`, {}, true))); const usableRows = rowsByPlan.reduce((sum, rows) => sum + rows.filter(row => String(row.ModelId || currentModel.id) === String(currentModel.id) && Number.isInteger(Number(row.ObjectRuntimeId)) && Number(row.ObjectRuntimeId) > 0).length, 0); if (!usableRows) throw new Error("The selected assemblies have no current viewer IDs. Select them again in the 3D Viewer and use Add current 3D selection."); await workspace.viewer.setObjectState(undefined, { color: "#8b9299", opacity: 1 }); const palette = ["#f97316", "#ec4899", "#8b5cf6", "#14b8a6", "#eab308", "#ef4444", "#06b6d4", "#84cc16"]; const coloured = new Set(); let colouredCount = 0; for (const [index, rows] of rowsByPlan.entries()) { const ids = colourIds(rows).filter(group => { group.objectRuntimeIds = group.objectRuntimeIds.filter(runtimeId => !coloured.has(`${currentModel.id}:${runtimeId}`)); group.objectRuntimeIds.forEach(runtimeId => coloured.add(`${currentModel.id}:${runtimeId}`)); return group.objectRuntimeIds.length; }); if (ids.length) { await workspace.viewer.setObjectState(ids, { color: palette[index % palette.length], opacity: 1 }); colouredCount += ids[0].objectRuntimeIds.length; } } status(`${colouredCount} assemblies coloured across ${selectedPlans.length} plan(s); other IFC assemblies are grey.`, "success"); }
