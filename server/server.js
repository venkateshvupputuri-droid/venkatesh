...
app.get("/api/plans/:planId/assemblies", planAuth, asyncRoute(async(req,res)=>{const r=await (await pool()).request().input("id",sql.Int,req.params.planId).query("SELECT pa.Id,pa.PlanId,pa.ModelId,pa.ObjectRuntimeId,pa.AssemblyGuid,pa.AssemblyName,pa.AssemblyMark,pa.Weight,pa.Grid,pa.Tos,pa.SequenceOrder,CONCAT(p.PlanNumber,'-',pa.SequenceOrder) SequenceCode FROM dbo.PlanAssemblies pa JOIN dbo.Plans p ON p.PlanId=pa.PlanId WHERE pa.PlanId=@id ORDER BY pa.SequenceOrder");res.json(r.recordset);}));
app.post("/api/plans/:planId/assemblies", planAuth, asyncRoute(async(req,res)=>{const items=Array.isArray(req.body.assemblies)?req.body.assemblies:[];const clean=[...new Map(items.filter(x=>x&&typeof x.guid==="string"&&x.guid.length<=100).map(x=>[x.guid,{guid:x.guid,modelId:String(x.modelId||"").slice(0,100)||null,runtimeId:Number(x.runtimeId)||null,name:String(x.name||"").slice(0,300)||null,mark:String(x.mark||"").slice(0,100)||null,weight:Number(x.weight)||0,grid:String(x.grid||"").slice(0,100)||null,tos:String(x.tos||"").slice(0,100)||null}])).values()];if(!clean.length)return res.status(400).json({error:"Select at least one valid assembly."});const tx=new sql.Transaction(await pool());await tx.begin(sql.ISOLATION_LEVEL.SERIALIZABLE);try{const plan=(await new sql.Request(tx).input("id",sql.Int,req.params.planId).query("SELECT PlanNumber FROM dbo.Plans WITH(UPDLOCK,HOLDLOCK) WHERE PlanId=@id")).recordset[0];if(!plan){await tx.rollback();return res.status(404).json({error:"Plan not found"});}let order=(await new sql.Request(tx).input("id",sql.Int,req.params.planId).query("SELECT ISNULL(MAX(SequenceOrder),0) maxOrder FROM dbo.PlanAssemblies WITH(UPDLOCK,HOLDLOCK) WHERE PlanId=@id")).recordset[0].maxOrder+1;const inserted=[];for(const x of clean){const dupe=await new sql.Request(tx).input("id",sql.Int,req.params.planId).input("guid",sql.NVarChar(100),x.guid).query("SELECT 1 found FROM dbo.PlanAssemblies WHERE PlanId=@id AND AssemblyGuid=@guid");if(dupe.recordset.length)continue;await new sql.Request(tx).input("id",sql.Int,req.params.planId).input("model",sql.NVarChar(100),x.modelId).input("runtime",sql.Int,x.runtimeId).input("guid",sql.NVarChar(100),x.guid).input("name",sql.NVarChar(300),x.name).input("mark",sql.NVarChar(100),x.mark).input("weight",sql.Decimal(12,3),x.weight).input("grid",sql.NVarChar(100),x.grid).input("tos",sql.NVarChar(100),x.tos).input("order",sql.Int,order).query("INSERT dbo.PlanAssemblies(PlanId,ModelId,ObjectRuntimeId,AssemblyGuid,AssemblyName,AssemblyMark,Weight,Grid,Tos,SequenceOrder) VALUES(@id,@model,@runtime,@guid,@name,@mark,@weight,@grid,@tos,@order)");inserted.push({guid:x.guid,name:x.name,sequenceCode:`${plan.PlanNumber}-${order++}`});}await tx.commit();res.json({success:true,inserted});}catch(e){try{await tx.rollback();}catch{}throw e;}}));
app.get('/api/projects/:projectId/models/:modelId/contractor',verifyProjectAccess,asyncRoute(async(req,res)=>{const r=await (await pool()).request().input('p',sql.NVarChar(100),req.params.projectId).input('m',sql.NVarChar(100),req.params.modelId).query('SELECT mc.IsLocked,mc.AssignedDate,c.ContractorName FROM dbo.ModelContractor mc JOIN dbo.Contractors c ON c.ContractorId=mc.ContractorId WHERE mc.ProjectId=@p AND mc.ModelId=@m');res.json(r.recordset[0]||null);}));
app.post('/api/projects/:projectId/models/:modelId/contractor',verifyProjectAccess,asyncRoute(async(req,res)=>{const c=Number(req.body.contractorId);const r=await (await pool()).request().input('p',sql.NVarChar(100),req.params.projectId).input('m',sql.NVarChar(100),req.params.modelId).input('n',sql.NVarChar(300),req.body.modelName||null).input('c',sql.Int,c).query("IF EXISTS(SELECT 1 FROM dbo.ModelContractor WHERE ProjectId=@p AND ModelId=@m AND IsLocked=1) THROW 50001,'Contractor is locked for this IFC.',1; MERGE dbo.ModelContractor t USING(SELECT @p p,@m m)s ON t.ProjectId=s.p AND t.ModelId=s.m WHEN MATCHED THEN UPDATE SET ContractorId=@c,ModelName=@n,IsLocked=1 WHEN NOT MATCHED THEN INSERT(ProjectId,ModelId,ModelName,ContractorId,IsLocked) VALUES(@p,@m,@n,@c,1);");res.json({success:true});}));
app.get('/api/projects/:projectId/models/:modelId/plans',verifyProjectAccess,asyncRoute(async(req,res)=>{const r=await (await pool()).request().input('p',sql.NVarChar(100),req.params.projectId).input('m',sql.NVarChar(100),req.params.modelId).query('SELECT PlanId,PlanNumber,PlanName FROM dbo.Plans WHERE ProjectId=@p AND ModelId=@m ORDER BY PlanNumber');res.json(r.recordset);}));
app.post('/api/projects/:projectId/models/:modelId/plans',verifyProjectAccess,asyncRoute(async(req,res)=>{const r=await (await pool()).request().input('p',sql.NVarChar(100),req.params.projectId).input('m',sql.NVarChar(100),req.params.modelId).input('mn',sql.NVarChar(300),req.body.modelName||null).input('no',sql.Int,Number(req.body.planNumber)).input('n',sql.NVarChar(200),req.body.planName||null).query('INSERT dbo.Plans(ProjectId,ModelId,ModelName,PlanNumber,PlanName) OUTPUT INSERTED.PlanId VALUES(@p,@m,@mn,@no,@n)');res.status(201).json({planId:r.recordset[0].PlanId});}));
