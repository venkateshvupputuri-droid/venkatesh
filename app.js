...
async function colourizePlan() { if (!selectedPlans.length) throw new Error("Select at least one plan first."); const rowsByPlan = await Promise.all(selectedPlans.map(plan => api(`/plans/${plan.PlanId}/assemblies`, {}, true))); const usableRows = rowsByPlan.reduce((sum, rows) => sum + rows.filter(row => String(row.ModelId || currentModel.id) === String(currentModel.id) && Number.isInteger(Number(row.ObjectRuntimeId)) && Number(row.ObjectRuntimeId) > 0).length, 0); if (!usableRows) throw new Error("The selected assemblies have no current viewer IDs. Select them again in the 3D Viewer and use Add current 3D selection."); await workspace.viewer.setObjectState(undefined, { color: "#8b9299", opacity: 1 }); const palette = ["#f6a623", "#4da3ff", "#d86bff", "#55c878", "#ff6f61", "#35c2c9", "#f2d04f", "#ef8f45"]; const coloured = new Set(); let colouredCount = 0; for (const [index, rows] of rowsByPlan.entries()) { const ids = colourIds(rows).filter(group => { group.objectRuntimeIds = group.objectRuntimeIds.filter(runtimeId => !coloured.has(`${currentModel.id}:${runtimeId}`)); group.objectRuntimeIds.forEach(runtimeId => coloured.add(`${currentModel.id}:${runtimeId}`)); return group.objectRuntimeIds.length; }); if (ids.length) { await workspace.viewer.setObjectState(ids, { color: palette[index % palette.length], opacity: 1 }); colouredCount += ids[0].objectRuntimeIds.length; } } status(`${colouredCount} assemblies coloured across ${selectedPlans.length} plan(s); other IFC assemblies are grey.`, "success"); }
